...
class ModelTests(TestCase):
    def setUp(self):
        ...
        self.project = Project.objects.create(
            name="Test Project", key="TST", created_by=self.user
        )
        self.sprint = Sprint.objects.create(name="Sprint 1", project=self.project)
        self.ticket = Ticket.objects.create(
            key="TST-1", title="First Ticket", project=self.project, reporter=self.user
        )
...
class SerializerTests(TestCase):
    ...
    def test_project_serializer_valid(self):
        data = {"name": "New Project", "key": "NEW", "description": "Description"}
